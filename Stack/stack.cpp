...
#ifdef USE_CANARIES
        wmemset((wchar_t *)(stack->data + stack->maxsize + CANARY_STACK_SIZE), (wchar_t)(DEFAULT_POISON), (newSize - stack->maxsize) * sizeof(elem_t) / sizeof(wchar_t));
#else
...
int checkStackValidity(stack_t *stack, const char *dumpPath, bool abortOnCorruption, bool silent) {
    ...
    if ((errno = stackOk(*stack)) != 1) {
        if (!silent) printf(ANSI_COLOR_RED "Stack have been corrupted: error code: %d - see the %s file for stack dump\n" ANSI_COLOR_RESET, errno, dumpPath);
